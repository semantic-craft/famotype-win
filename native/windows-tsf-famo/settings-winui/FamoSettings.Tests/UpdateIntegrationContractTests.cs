...
public sealed class UpdateIntegrationContractTests
{
    ...
    [Fact]
    public void ReleaseMetadataIsPreparedFor158()
    {
        string build = File.ReadAllText(RepoFile(
            "native/windows-tsf-famo/installer/build-installer.ps1"));
        ...
        Assert.Contains("[string] $AppVersion = '1.5.8'", build);
        Assert.Contains("[string] $AppVersion = '1.5.8'", appcast);
        Assert.Contains("#define AppVersion  \"1.5.8\"", installer);
        Assert.Contains("\"versionInfo\": \"1.5.8\"", sbom);
    }
