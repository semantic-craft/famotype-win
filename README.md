...
**更新**：设置面板启动后默认每天最多后台检查一次；可在「关于」页关闭，也可随时手动检查。
发现新版后由你确认，法墨才会下载并用内置 EdDSA 公钥验签，再以可见进度启动事务安装器；
不会强关桌面程序或强制重启。由于安装器尚未做 Authenticode 签名，安装时仍会出现
「未知发布者」UAC。未带更新模块的旧版本需要先手动安装一次带自动更新的新版。
...
- **现代化设置面板**——侧栏单字徽标导航（键 / 捷 / 候 / 皮 / 部 / 关）+ 分组卡片，
  不用翻 YAML。
- **7 套学院皮肤**——荔园红、胡佛红、珞珈青、嘉庚蓝、伊利诺伊、伊利诺伊火焰、纽约紫，明暗自适应、即时换肤。
